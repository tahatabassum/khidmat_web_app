...
export const submitProviderReview = async (bookingId: string, providerId: string, rating: number) => {
  ...
  // 2. Save customer rating on the booking
  ...
  // 3. Get all completed bookings for this provider to calculate new averages
  const allBookings = await queryCollectionDocs('bookings', 'providerId', '==', providerId);
  const completedBookings = allBookings.filter((b: any) => b.status === 'completed');
  const reviewedBookings = completedBookings.filter((b: any) => b.customerRating !== undefined && b.customerRating !== null);
  ...
  // 4. Load baseline statistics of the provider profile to blend baseline with new reviews
  const providerProfile = await getDocument('providers', providerId) as any;
  let finalRating = rating;
  
  if (providerProfile) {
    const baselineJobs = Math.max(providerProfile.totalJobs || 5, 1);
    const baselineRating = providerProfile.rating || 4.7;
    
    // Weighted blend algorithm
    const totalWeight = baselineJobs + reviewedBookings.length;
    finalRating = ((baselineRating * baselineJobs) + sumRatings) / totalWeight;
  }
  
  // Format rating to 1 decimal place (e.g. 4.8)
